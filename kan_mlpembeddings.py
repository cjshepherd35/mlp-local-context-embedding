...
class SplineActivation(nn.Module):
    def __init__(self, grid_size=5, grid_range=[-5.0, 5.0], k=3):
        super().__init__()
        ...
        pad_left = torch.linspace(grid_range[0] - k * step, grid_range[0] - step, k)
        pad_right = torch.linspace(grid_range[1] + step, grid_range[1] + k * step, k)
        grid = torch.cat([pad_left, grid, pad_right])
        self.register_buffer("grid", grid)
        
        # spline coefficients (trainable) - shared across all neurons in the layer
        self.spline_weight = nn.Parameter(torch.randn(grid_size + k) * 0.1)

    def b_splines(self, x):
        grid = self.grid
        k = self.k
        # Add a dimension for basis coefficients
        x = x.unsqueeze(-1)
        # degree 0: indicator function for each interval [t_i, t_{i+1})
        bases = ((x >= grid[:-1]) & (x < grid[1:])).to(x.dtype)
        
        # recurrence for degree 1 to k
        for p in range(1, k + 1):
            w1 = (x - grid[:-(p+1)]) / (grid[p:-1] - grid[:-(p+1)] + 1e-8)
            w2 = (grid[p+1:] - x) / (grid[p+1:] - grid[1:-p] + 1e-8)
    ...
    def forward(self, x):
        # x: (..., num_activations)
        bases = self.b_splines(x) # (..., grid_size + k)
        # multiply by shared weights and sum over basis dimension
        out = (bases * self.spline_weight).sum(-1)
        return out

class FeedForward(nn.Module):
    def __init__(self, n_embed):
        super().__init__()
        # 4*n_embed neurons in the hidden layer
